...
ll calc(int id, bool tight, int number, int sumd, vector <int> digit){
    if (id < 0) return ((number == 0) & (sumd == 0));
    if (!tight && f[id][number][sumd] != -1) return f[id][number][sumd];
    int c = (tight) ? digit[id] : 9;
    ll cnt = 0;
    FOR(i, 0, c){
        bool new_tight = (i == digit[id]) ? tight : 0;
        cnt += calc(id - 1, new_tight, (number * 10 + i) % k, (sumd + i) % k, digit);
    }
    if (tight) return cnt;
    return (f[id][number][sumd] = cnt);
}
